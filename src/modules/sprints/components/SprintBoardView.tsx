...
import type { ProjectOption } from '@/modules/projects/projects.types';
import type { SprintBoard } from '../sprints.types';
import {
  DEFAULT_SPRINT_CAPACITY_SP,
  ROADMAP_MILESTONES,
  SPRINT_CAPACITY_ALERT_PERCENT,
} from '../sprints.constants';

interface SprintBoardViewProps extends SprintBoard {
  projects: ProjectOption[];
}

export function SprintBoardView({ currentSprint, backlogTasks }: SprintBoardViewProps) {
  const { setSprintGuardModalOpen } = useUIStore();

  const totalPoints = currentSprint?.tasks.reduce((sum, t) => sum + t.storyPoints, 0) || 0;
  const capacity = currentSprint?.capacitySp || DEFAULT_SPRINT_CAPACITY_SP;
  const capacityPercent = Math.min(100, Math.round((totalPoints / capacity) * 100));
  const isOverloaded = capacityPercent > SPRINT_CAPACITY_ALERT_PERCENT;
          ...
          <Box>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
              ...
              <Typography
                ...
                sx={{
                  fontWeight: 600,
                  color: isOverloaded ? 'error.main' : 'success.main',
                }}
              >
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              {ROADMAP_MILESTONES.map((milestone) => (
                <Box
                  ...
                  sx={{ pl: 2, borderLeft: `3px solid ${milestone.isCurrent ? 'var(--color-primary)' : '#cbd5e1'}` }}
                >
                  <Typography
                    variant="caption"
                    sx={{ fontWeight: 700, color: milestone.isCurrent ? 'primary.main' : 'text.disabled' }}
                  >
                    {milestone.period}
                  </Typography>
                  <Typography
                    variant="body2"
                    sx={{ fontWeight: milestone.isCurrent ? 700 : 600, fontSize: '0.8125rem', mt: 0.25 }}
                  >
                    {milestone.title}
                  </Typography>
