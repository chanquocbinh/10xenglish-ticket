...
import { swapSprintTaskAction } from '@/modules/tasks/tasks.actions';
import type { SprintTaskOption } from '@/modules/tasks/tasks.types';
...
export function SprintGuardModal({
  ...
  const [selectedProjCode, setSelectedProjCode] = useState<string>(
    projects.find((p) => p.code === activeCode)?.code ?? projects[0]?.code ?? '',
  );
