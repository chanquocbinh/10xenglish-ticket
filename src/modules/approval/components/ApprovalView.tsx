...
import { signOffAction } from '@/modules/approval/approval.actions';
...
export function ApprovalView({
  ...
  const canApproveTicket = hasPermission(currentUser, 'tickets.approve');
  const canApproveTask = hasPermission(currentUser, 'tasks.approve');

  const handleApprove = async (type: 'TICKET' | 'TASK', id: string, name: string) => {
    if (type === 'TICKET' ? !canApproveTicket : !canApproveTask) {
      alert('Bạn không có quyền phê duyệt nghiệm thu mục này!');
      return;
    }
