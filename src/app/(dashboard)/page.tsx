...
export default async function DashboardPage() {
  ...
  // Fetch metrics
  const pendingBugsCount = await prisma.ticket.count({
    where: {
      ...projectWhere,
  ...
  const blockerBugsCount = await prisma.ticket.count({
    where: {
      ...projectWhere,
      status: { in: ['NEW', 'IN_PROGRESS'] },
      severity: 'BLOCKER',
  ...
  const pendingApprovalCount = await prisma.task.count({
    where: {
      ...projectWhere,
  ...
  const totalTasks = await prisma.task.count({ where: projectWhere });
  const doneTasks = await prisma.task.count({ where: { ...projectWhere, status: 'DONE' } });
          ...
          <CardContent sx={{ p: 3, '&:last-child': { pb: 3 } }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>
                    LMS & Điểm Danh Giáo Viên (Project 1)
                  </Typography>
              ...
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>
                    CRM Tư Vấn & Tuyển Sinh (Project 2)
                  </Typography>
              ...
              <Box>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                  <Typography variant="body2" sx={{ fontWeight: 600 }}>
                    Kế Toán Học Phí & VietQR (Project 2)
                  </Typography>
